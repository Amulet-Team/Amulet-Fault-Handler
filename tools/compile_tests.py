...
def main() -> None:
    ...
    if subprocess.run(
        ...
        raise RuntimeError("Error configuring test-amulet-faulthandler")
    if subprocess.run(
        ["cmake", "--build", "build", "--config", "RelWithDebInfo"]
    ).returncode:
        raise RuntimeError("Error building test-amulet-faulthandler")
    if subprocess.run(
        ["cmake", "--install", "build", "--config", "RelWithDebInfo"]
    ).returncode:
        raise RuntimeError("Error installing test-amulet-faulthandler")
